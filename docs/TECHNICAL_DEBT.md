...
- **Affected components:** `src/cli/video-metadata.ts` (`changeset`/`batch`/`channel` namespaces added 2026-09-20 — CLI parity for every MCP read/propose/create tool below), `src/mcp/server.ts` (read/propose-only `changeset_*`/`batch_*`/`localization_import_preview` tools added 2026-09-20 Phase 7 slice 1; `channel_sync`/`channel_list`/`channel_video_list` added 2026-09-20 as `BL-008`; `changeset_create_from_import` added 2026-09-20; a restricted/read-only server mode added 2026-09-20; no apply-class Change Set/Batch tool exists yet on either interface).
