...
## RISK-10 — `src/lib/db.ts` duplicates the `LedgerStatus`/`AttemptPhase`/`AttemptOutcome` type definitions instead of importing them from `src/lib/batches/contracts.ts`

- **Affected components:** `src/lib/db.ts` (persistence-layer type aliases), `src/lib/batches/contracts.ts` (the authoritative domain definitions).
- **Current behavior:** `db.ts` declares its own copies of these union types (needed because it predates, and is imported by, `batches/contracts.ts`, so a direct import would be circular). When Slice 2 added `DRY_RUN_COMPLETE` and Slice 3 added `AWAITING_EXECUTION` to the authoritative definition in `contracts.ts`, `db.ts`'s copy was not updated in the same edit and only failed at `tsc --noEmit` once a test happened to pass one of the new literal values through a `db.ts` function signature -- it was not caught by `npm test` (which does not typecheck; `tsx` only transpiles) or by casual review.
- **Actual risk:** A future status value added to one copy and not the other reintroduces the same silent drift, and, if the missing value never happens to appear in a function argument in any test, may not surface as a compile error until `npm run build` (or, worse, until a real deployment) -- `npm test` alone cannot catch it, per this same investigation.
- **Existing mitigation:** A code comment in `db.ts` now states the copies must be kept in sync by hand. `npm run build`'s `tsc` pass, run before every task is considered complete per `AGENTS.md` §E, will catch a divergence that manifests as a type error (as it did here) -- but only if some call site actually exercises the new value; it does not detect an *unused* stale literal sitting harmlessly in `db.ts`'s union.
- **Required remediation:** Either (a) restructure so `db.ts` imports these three types from `batches/contracts.ts` (requires resolving or accepting the import-direction constraint, e.g. moving the shared literals to a small types-only module both files import), or (b) add a lightweight compile-time or test-time assertion that the two type unions are structurally identical (e.g. a mutual-assignability check in a `.test-d.ts`-style file, or a runtime test that enumerates both and diffs them) so a future addition to one and not the other fails loudly and immediately, not only when a test happens to touch the new value.
- **Acceptance criteria:** A deliberately-reintroduced mismatch (e.g. temporarily removing one status from one of the two type unions) is caught by an automated check without requiring a human to notice a `tsc` error buried in unrelated output.
- **Gate(s):** none blocking today (Phase 5 is still pre-Gate-B); should be closed before Slice 4 adds further status-adjacent types for the real write adapter, to avoid compounding the drift.
- **Approval required from:** none -- this is an internal consistency fix, not a product/security decision.
- **Status:** OPEN — found and worked around (not fixed at the root) during Slice 3's 2026-09-17 Foundation-safety verification; the immediate divergence was corrected by hand, but the structural cause (two independently-maintained copies) remains.
...
| RISK-10 | Duplicated LedgerStatus/AttemptPhase/AttemptOutcome type definitions can silently drift | none blocking yet | OPEN |
