...
- **Affected components:** `src/cli/video-metadata.ts` (still zero `changeset_*`/`batch_*`/`channel_sync` commands — the CLI portion of this risk is entirely untouched), `src/mcp/server.ts` (read/propose-only `changeset_*`/`batch_*`/`localization_import_preview` tools added 2026-09-20 Phase 7 slice 1; `channel_sync`/`channel_list`/`channel_video_list` added 2026-09-20 as `BL-008`; no apply-class Change Set/Batch tool exists yet).
- **Current behavior:** MCP now covers every read/propose-only operation RISK-04 originally named: `changeset_list`, `changeset_get`, `localization_import_preview`, `batch_list`, `batch_get`, `channel_sync`, `channel_list`, `channel_video_list` — all reusing the same interface-agnostic `createChangeSetCore()`/`createBatchCore()`/`createChannelSyncCore()` factories the Web UI's own API routes already use, no new persistence or validation logic introduced. `channel_sync` is gated by the same device-availability mutation gate as `apply`/`playlistCreate` etc. (it writes to the local `channels`/`videos` tables, even though it never writes to YouTube). Approving/rejecting/creating/executing a Change Set or Batch through MCP still requires the Web UI. **The CLI has none of this at all** — no CLI command exists for channel sync, localization, change sets, or batches.
- **Actual risk:** The future operations agent (Codex) is meant to operate exclusively through MCP/API (`docs/PROJECT_SPEC.md` §26, this task's development/operations separation). Codex can now sync a channel and inspect Change Sets/Batches/localizations via MCP, but still cannot approve, create, or execute anything — the propose→approve→execute pipeline remains human-Web-UI-only past the read/propose step.
...
- **Acceptance criteria:** Met for the MCP read/propose surface (17 new MCP tool tests across two tasks, `src/mcp/server.test.ts`, proving stable JSON shapes, `DomainError`-shaped errors, channel-ownership verification via `requireBatchForChannel`/`requireChannel` rather than a bare id lookup, and correct mutation-gate wiring for `channel_sync` specifically). Still needed for CLI parity and any future apply-class tool.
...
- **Status:** PARTIALLY RESOLVED (2026-09-20) — every read/propose-only MCP tool RISK-04 originally named now exists; CLI parity and any apply-class Change Set/Batch MCP tool remain OPEN.
