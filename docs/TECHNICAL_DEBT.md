...
- Fresh remote conflict detection (closes RISK-03).
- Channel identity verification reusing `write-context.assertWriteChannel` (already implemented, must be reused not reimplemented).
- Immutable backup before any write batch (closes part of RISK-09).
- Approval integrity: an approval must be provably tied to the exact payload it authorized (already implemented for local state, §6.9 of `docs/ARCHITECTURE.md`; must extend to the write payload itself).
- Dry-run support for the batch pipeline.
- Durable audit log (closes part of RISK-09).
- Per-item execution ledger supporting idempotent resume (closes part of RISK-09).
- Post-write remote verification.
- All relevant security issues from this register either remediated or explicitly accepted via a documented decision (an ADR or an explicit sign-off recorded here).
...
- **Existing mitigation:** `Content-Length` pre-check (rejects the common case — every normal browser file upload sends this header); post-parse `MAX_WORKBOOK_BYTES`/`MAX_LOCALIZATION_ROWS` checks bound damage from a request that does slip through.
- **Required remediation:** Determine whether Next.js's underlying request architecture (the Web `Request`/`ReadableStream` it wraps) supports bounded, streaming body consumption in this version (`node_modules/next/dist/docs/` per the project's Next.js agent-warning block) so the size limit can be enforced against actual bytes received, not just a declared header. If not directly supported, implement a manual streaming reader with a hard byte cap before handing data to `exceljs`.
...
- **Actual risk:** Without this infrastructure, enabling real bulk localization writes would have no rollback path, no tamper-evident record of what was changed and by whom, and no safe way to resume an interrupted batch without risking duplicate or inconsistent writes.
...
- **Required remediation:** Implement the full pipeline from `docs/PROJECT_SPEC.md` §19–25 (immutable backup → diff → approval [done, Phase 4] → dry-run → apply → verify → audit) with a per-item ledger supporting idempotent resume, reusing `write-context.assertWriteChannel` for identity (`docs/ARCHITECTURE.md` §11).
- **Acceptance criteria:** The five acceptance tests defined in `docs/PROJECT_SPEC.md` §53–57 (full happy path, interrupted-batch resume, wrong-channel fail-closed, conflict detection, existing-localizations-preserved) all pass against mocked YouTube adapters, with zero writes to a real channel in any automated test.
- **Gate(s):** `BLOCKS_PHASE_5_WRITES`, `BLOCKS_OPERATIONS_RELEASE`.
- **Approval required from:** project owner (Gate B sign-off, see "Release-readiness checkpoints" above).
