...
- **Ответственность:** дашборд оператора (`/dashboard`) с вкладками Manual / Rules / Sync / Localizations / **Batches** (новая, Phase 5 completion task).
- **Файлы:** `src/app/page.tsx` (страница входа), `src/app/dashboard/page.tsx`, `src/components/{manual-mode,rule-form,rule-list,run-button,session-provider,channel-sync,localization-manager,change-set-review,batch-manager}.tsx`.
...
- **Read/Write:** вкладки **Sync** и **Localizations** — читают YouTube только через уже синхронизированные данные (Localizations вообще не вызывает YouTube API); XLSX-импорт и approve/reject в Localizations (Phase 4) — только локальная запись в БД, тоже без вызовов YouTube API; вкладка **Manual** — может писать в YouTube (add/remove из плейлиста, создание плейлиста). Вкладка **Batches** — выбор одобренных изменений → создание Batch (принудительно dry-run на уровне API-роута, независимо от тела запроса) → просмотр неизменяемого состава, статусов ledger-строк, отчёта об ошибках и audit-трейла → кнопка запуска dry-run превью. **Нет и намеренно не будет кнопки/действия «Apply»/live-запуска** — реальные записи отключены серверным барьером (RISK-09/Gate B), показывать нефункциональную или небезопасную кнопку запрещено явным требованием задачи.
- **Важные ограничения безопасности:** каждый API route, к которому обращается UI, сам проверяет сессию (`getServerSession`) и (для write) guardrail канала — UI не является границей безопасности. Для Batches-роутов channel-scoping обеспечивает новая `requireBatchForChannel` (AGENTS.md §F).
