...
| §28 | Batched/centralized API calls; recognize and do not blindly retry quota exhaustion | AC-QUOTA-01a, AC-QUOTA-01b, AC-QUOTA-02 |
...
### AC-QUOTA-01 — Fresh remote-state fetches are batched where safe, never at the cost of the mandatory per-video pre-write check (revised 2026-09-18, sixth review round — see §7 for the ambiguity this replaces)

Split into two sub-scenarios, per the project owner's 2026-09-18 approval of a narrow editorial clarification: the original single scenario's text ("the fresh-fetch step issues at most 2 calls, not 75") did not yet distinguish between an informational overview pass and the mandatory, safety-critical, immediately-before-send check -- that distinction was introduced later, during Slice 2 implementation (RISK-03/AC-MERGE-02), and is made explicit here without changing what either sub-scenario actually requires of the implementation already built and tested against it.

#### AC-QUOTA-01a — Preliminary batch-wide overview is batched, and is never the source of a per-video write decision

- **Requirement reference:** §28.
- **Preconditions:** A batch covering 75 distinct videos.
- **Fixed test inputs:** 75 video ids, one call to the batch's preliminary-overview fetch dependency, using the existing `getVideosMetadataContextBatch`-style chunking (≤50 ids per underlying `videos.list` call, matching the established Phase 2 pattern) internally.
- **Expected result:** The preliminary overview issues at most `ceil(75/50) = 2` underlying `videos.list` calls, not 75 (verified at the adapter/chunking layer) and exactly one call to the service-level preliminary-fetch dependency per batch (verified at the service layer) -- not one call per video at either layer. Its result is never used to build a per-video write payload, conflict decision, `defaultLanguage` check, or backup decision (this is the same guarantee `AC-MERGE-02` already independently requires and tests; restated here for `AC-QUOTA-01`'s own traceability).
- **Prohibited side effects:** No per-video individual overview-fetch loop. Any code path that lets the preliminary overview's result feed a per-video safety decision.
- **Verification method:** Automated (assert call count on the mocked batched-fetch function at both the service-dependency layer and, separately, the underlying chunking layer).
- **Pass/fail criteria:** PASS iff the preliminary overview issues ≤2 underlying `videos.list` calls for 75 videos and its result is structurally never used for any per-video decision. FAIL on a per-video overview-fetch loop, or on any evidence the overview's result reached a write/conflict/backup decision.

#### AC-QUOTA-01b — The mandatory pre-write fetch remains one call per video, restated (not a quota-savings target)

- **Requirement reference:** §21, §28; RISK-03; `AC-MERGE-02`.
- **Preconditions:** The same 75-video batch as AC-QUOTA-01a.
- **Fixed test inputs:** The same 75 videos, observed through the mandatory single-video fresh-fetch dependency each video's own preparation/send-time re-check uses.
- **Expected result:** This fetch remains exactly one call per video -- 75 calls for 75 videos -- **by design, not as a gap**. It is never satisfied from the preliminary overview (AC-QUOTA-01a) or from any other batched/cached source: `RISK-03`/`AC-MERGE-02` require a genuinely fresh, immediately-before-use read for the value every write payload, conflict check, and `defaultLanguage` check is built from, and a batch-wide snapshot's per-video freshness degrades unevenly depending on how far into a (possibly long-running, possibly concurrent) batch a given video's turn falls. This scenario exists specifically so that a future reader cannot mistake `AC-QUOTA-01a`'s batching for license to also batch this fetch.
- **Prohibited side effects:** Any change that routes this fetch through the preliminary overview, a cache, or any source other than a fresh, single-video call made immediately before that video's own use of it.
- **Verification method:** Automated (assert the mandatory fetch dependency is called once per video, independently of and in addition to the preliminary overview's own call count).
- **Pass/fail criteria:** PASS iff the mandatory fetch is called exactly once per video regardless of batch size, with zero reliance on the preliminary overview's result. FAIL if any video's write/conflict/`defaultLanguage` decision is built from anything other than that video's own fresh, single-video fetch.
...
- **AC-QUOTA-01's "the fresh-fetch step" ambiguity (fixed, sixth review round, 2026-09-18, project-owner-approved narrow editorial correction — see `docs/TECHNICAL_DEBT.md` RISK-13 for the full analysis that prompted it):** the original AC-QUOTA-01 asked for "the fresh-fetch step" to issue ≤2 calls for 75 videos, written before Slice 2's implementation split a single fresh-fetch concept into two genuinely different mechanisms: an informational preliminary overview (safe to batch) and the mandatory, immediately-before-send, per-video fetch that `RISK-03`/`AC-MERGE-02` require to stay unbatched for safety. Read literally, the original text could be misread as requiring — or as having been satisfied by — batching the mandatory fetch, which would silently reopen RISK-03. AC-QUOTA-01 is now split into `AC-QUOTA-01a` (the preliminary overview, batched, ≤2 calls for 75 videos, never a source of any per-video decision) and `AC-QUOTA-01b` (the mandatory per-video fetch, restated explicitly as one call per video by design, never a quota-savings target). This is a wording clarification, not a safety-requirement change: the implementation and its tests already conformed to `AC-QUOTA-01b`'s intent before this edit; the edit only makes explicit, in the acceptance document's own text, a distinction that previously existed only in code comments.

---

## 8. Status

**APPROVED (2026-09-17, fifth review round; sixth review round, 2026-09-18, applied without reopening approval) as the acceptance contract for the start of Phase 5 preparation and implementation.** It incorporates: the project owner's 2026-09-17 answers to OQ-1..OQ-6 and seven additionally required scenarios from the first review round (A-G); the second review round's batch/approval-integrity fix (AC-BATCH-03), bounded reconciliation procedure (§0.F), and durable two-phase attempt-intent model (§0.C, AC-ATTEMPT-03/04); the third review round's fixes — the AC-BATCH-02/AC-BATCH-03 contradiction removed, §0.F/AC-TIMEOUT-01 corrected so that no combination of reconciliation reads ever authorizes an automatic retry, and the duplicated Verification method/Pass-fail text removed from AC-TIMEOUT-01; the fourth review round's fixes — AC-ATTEMPT-01/02's fixture corrected to use two definitively-known transient errors instead of a timeout (avoiding any implication that a timeout may be retried directly), `INTENDED`'s semantics clarified as durable intent only, never proof of transmission (§0.C, AC-ATTEMPT-04's two sub-cases), and reconciliation's causation limits made explicit so audit records never attribute an unconfirmed cause to a specific attempt's own API call (§0.F's causation note, AC-AUDIT-05, AC-CRASH-01 updated accordingly); the fifth review round's three project-owner-mandated editorial fixes — the AC-TIMEOUT-02/AC-BATCH-03 contradiction removed, AC-AUDIT-01's `v5` fixture decoupled from AC-ATTEMPT-01's, and AC-RESUME-01 sharpened to enumerate every per-video state at interruption; a same-day post-approval editorial correction removing a leftover duplicated Pass/fail-criteria block from AC-TIMEOUT-02 (mechanical cleanup, no semantic change); and the **sixth review round (2026-09-18, project-owner-approved narrow editorial correction, RISK-13)** — AC-QUOTA-01 split into AC-QUOTA-01a (batched preliminary overview) and AC-QUOTA-01b (mandatory unbatched per-video fetch, restated), clarifying, not weakening, the existing RISK-03/AC-MERGE-02 safety requirement. **This sixth round is a wording clarification only — it does not reopen or require re-approval of any other part of this document, per the project owner's own framing of the request.**
