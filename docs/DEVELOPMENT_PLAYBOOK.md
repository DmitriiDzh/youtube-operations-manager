...
1. **Low-level READ call** goes in `src/lib/youtube.ts`. **A low-level WRITE call (anything that can mutate data on YouTube — `videos.update`, `playlists.insert/update/delete`, `playlistItems.insert/delete`, or any future mutating method on any resource) goes in `src/lib/youtube-write-gateway/` instead, never in `youtube.ts`** (owner instruction, 2026-09-21 -- "единственный путь как информация может попасть в 'релиз'"; see §6.5 and `AGENTS.md` §G). `youtube.ts` is the single wrapper for reads only; the gateway is the single, mechanically-enforced funnel for writes (`src/lib/youtube-write-gateway/gateway-inventory.test.ts` fails the build if a mutating call appears anywhere else). If you need a new read (e.g. a new `part` on `videos.list`, or a new resource type), add a function to `youtube.ts`, not in a domain module's adapter directly.
