...
- **YouTube client:** `src/lib/youtube-read-gateway/` (reads, barrel `index.ts` + `data-api.ts`, formerly `src/lib/youtube.ts` — see §6.4 point 1 and `docs/decisions/0007-youtube-read-gateway.md`) and `src/lib/youtube-write-gateway/` (writes, `docs/decisions/0005-youtube-write-gateway.md`) — the **only** two low-level `googleapis` wrappers. Every domain module's `adapters/youtube-api.ts` is a thin wrapper around functions from these gateways' barrels, never a direct import of a gateway's internal child module.
...
**A new module never**: creates a second YouTube client abstraction (extend `src/lib/youtube-read-gateway/data-api.ts` for reads, or `src/lib/youtube-write-gateway/` for writes, instead), duplicates `write-context`'s guardrail, or writes its own SQLite connection (add tables/functions to `src/lib/db.ts`, wrap them in `adapters/store.ts`).
...
1. **Low-level READ call** goes in `src/lib/youtube-read-gateway/data-api.ts` (YouTube Data API v3 -- the read gateway's child module for this API category, owner instruction, 2026-09-22 -- "Все запросы на получения данных должны идти через него и никак иначе"; see `docs/decisions/0007-youtube-read-gateway.md`), re-exported through that gateway's own `index.ts` barrel -- never imported by a caller directly from `data-api.ts` (`src/lib/youtube-read-gateway/read-gateway-inventory.test.ts` fails the build if a caller does). **A low-level WRITE call (anything that can mutate data on YouTube — `videos.update`, `playlists.insert/update/delete`, `playlistItems.insert/delete`, or any future mutating method on any resource) goes in `src/lib/youtube-write-gateway/` instead, never in the read gateway** (owner instruction, 2026-09-21 -- "единственный путь как информация может попасть в 'релиз'"; see §6.5 and `AGENTS.md` §G). The read gateway is the single wrapper for reads only; the write gateway is the single, mechanically-enforced funnel for writes (`src/lib/youtube-write-gateway/gateway-inventory.test.ts` fails the build if a mutating call appears anywhere else). If you need a new read (e.g. a new `part` on `videos.list`, or a new resource type), add a function to `data-api.ts` (or a new category-specific child module, re-exported from the barrel), not in a domain module's adapter directly.
...
6. **Adapter:** wrap the gateway function(s) — imported from `@/lib/youtube-read-gateway`, the barrel, never a specific child module — in your module's `adapters/youtube-api.ts`, injected into `services.ts` via `ServiceDependencies`.
...
8. **Tests:** unit-test the adapter's chunking/pagination math directly against a **mocked real-shaped** `youtube_v3.Youtube` client (see `src/lib/youtube-read-gateway/data-api.test.ts`'s "120 ids → 3 calls of 50/50/20" style test) — not just indirectly through the service layer, so the actual API-shape assumptions are verified.

---

## 6.5 Adding safe YouTube write operations

This section documents the **mandatory future pipeline** for bulk localization writes. As of the Phase 5 completion task (2026-09-18, following Slice 5): all 12 steps have a real (non-fake) implementation, all three gaps Slice 4 found and flagged (configurable concurrency, batched preliminary fetch, the full 100-video resume scenario) are closed, and the two remaining automated-acceptance gaps Slice 5 itself left open (`AC-CONCURRENCY-03`'s full two-batch race, `AC-E2E-01`'s single integrated scenario) are now closed too — see `docs/TECHNICAL_DEBT.md` RISK-09's progress notes for exact scope and honest caveats (the quota-batching fix reduces the preliminary pass's own call count, not the mandatory per-video fresh-fetch count; see RISK-13 for a documented tension in AC-QUOTA-01's own wording). A minimal Web UI/API for Batches now exists (`src/app/api/channels/[channelId]/batches/**`, `src/components/batch-manager.tsx`) — so `src/lib/batches/` **is** now legitimately imported from API routes, unlike before. **Corrected 2026-09-21 (this paragraph previously claimed no code path could ever reach a real `videos.update` call — that stopped being true as of BL-045, 2026-09-20, and this sentence was left stale; do not trust a "no code path can reach X" claim in this file without checking the current state of the barrier it describes):** the real `WriteExecutor` (`src/lib/batches/adapters/write-executor.youtube.ts`) is owner-gated, not unconditionally blocked — see the Settings tab "Live writes" toggle (`docs/TECHNICAL_DEBT.md` RISK-09, defaults off every session) and the two-layer barrier this section's own doc comments describe (`write-executor.ts`'s construction gate, `write-executor.youtube.ts`'s call-time re-check). **Single-funnel refactor, 2026-09-21 (owner instruction — "единственный путь как информация может попасть в 'релиз'"):** every physical outbound YouTube write call in this repository — bulk (Batches) and single-item (`apply`, `playlist_*`) alike — now goes through `src/lib/youtube-write-gateway/`, the one module allowed to call `videos.update`/`playlists.*`/`playlistItems.*`; see `docs/decisions/0005-youtube-write-gateway.md` and `AGENTS.md` §G. `src/lib/youtube-write-gateway/gateway-inventory.test.ts` fails the build if any other file calls a mutating `youtube_v3` method directly. The gateway's own `assertLiveWritesAuthorized` is the single, shared implementation of the "live writes" policy check — `write-executor.youtube.ts` calls it for Batches (Layer 2, unchanged in design), and the single-item `apply`/`playlist_*` adapters (`video-metadata/adapters/youtube-api.ts`, `video-details/adapters/youtube-api.ts`, `playlist-management/adapters/youtube-api.ts`) now call it too, immediately before their own gateway write call — closing a real gap this refactor's own review found (those single-item paths previously had the identity guardrail but no live-write barrier at all).
...
| 9. Apply | **IMPLEMENTED, but gated** (bulk: `executeBatch`/`executeWithRetry` drive real attempt cycles with bounded retry against the real `WriteExecutor`, which itself reuses `applyVideoMetadataUpdate`; owner-gated by the Settings tab "Live writes" toggle, not unconditionally blocked); IMPLEMENTED (single-item, same `applyVideoMetadataUpdate`, gated the same way) | `src/lib/batches/services.ts`, `adapters/write-executor.youtube.ts`; `src/lib/youtube-write-gateway/` has the actual write call for both paths |
...
2. **Channel-context validation is not automatic — and it is now two distinct checks, not one.** (a) A route that takes a `channelId` path parameter must have its underlying service verify the requested *resource* actually belongs to that channel. See `src/lib/changesets/services.ts`'s `requireChangeSet()`, which checks `changeSet.channelId === channelId` and throws `not_found` on mismatch — this prevents accessing a change set through a forged/mismatched `channelId` in the URL. (b) Separately, per `docs/decisions/0004-active-channel-read-scoping.md` (2026-09-20, closes `docs/TECHNICAL_DEBT.md` RISK-02): the `channelId` itself must be the caller's currently **active** channel — call `channelAccess.assertActiveChannel({ userId, channelId })` (`src/lib/channel-access`, `createChannelAccessCore()`) right after resolving the session, before either calling the service or doing check (a). A route that only does (a) still leaks every locally-known channel's data to any authenticated session, exactly like RISK-02 described — see any route under `src/app/api/channels/[channelId]/` for the pattern (e.g. `batches/[batchId]/route.ts`, which does both (b) then (a) in that order).
...
3. **Credential resolution:** most tools accept an optional `credentialRef`; when omitted, fall back to `resolveEffectiveCredentialRef` from `cli-auth` (whoever is the locally active user). `changesets/`/`batches/` tools make no YouTube calls at all, but as of 2026-09-20 they **do** need the resolved `userId` for the active-channel check (point 6 below) — resolve `credentialRef` even for an otherwise YouTube-call-free tool.
...
6. **Every MCP tool — read, local-mutation, or remote-mutation alike — enforces authentication, authorization, and channel scope**, the same way every API route must (§6.6, point 2 and point 8). Resolve the active identity (`credentialRef`/`resolveEffectiveCredentialRef`), then both: (a) call `channelAccessCore.assertActiveChannel({ userId, channelId })` to confirm the requested `channelId` is this session's active channel (`docs/decisions/0004-active-channel-read-scoping.md`), and (b) verify the requested `channelId` actually owns the resource being read or mutated (e.g. a `changeSetId` must be checked against its `channelId`, mirroring `requireChangeSet()`'s check in `services.ts`) — an MCP tool is a network-adjacent entry point exactly like an API route, not an inherently trusted internal call. See `changesetList`/`batchGet` in `src/mcp/server.ts` for the pattern.
...
6. **Wide tables (project owner's standing rule, 2026-09-20):** a table's own column widths should never be squeezed below a readable minimum just to fit a narrow window -- wrap the `<table>` in a `<div className="overflow-x-auto">` and give the `<table>` itself a `min-w-[Npx]` (the sum of every fixed `<col>` width plus a reasonable minimum for the one flexible column) so a narrow viewport scrolls horizontally instead of compressing columns unreadably. This is the direct counterpart to `AppShell`'s own width-agnostic content area (§6.9 above, no shell-level `max-w` cap) -- a table-heavy tab is free to stretch to fill a *wide* viewport, and free to horizontally scroll rather than break on a *narrow* one. See `content-manager.tsx` (`min-w-[720px]`), `languages-manager.tsx` (`min-w-[600px]`), and `batch-manager.tsx` (`min-w-[480px]`) for the current examples -- apply the same wrapper to any new table.
7. **Long-running operations:** disable the triggering button and show a busy label (`"Parsing..."`, `"Creating..."`) rather than blocking the whole page; do not freeze the UI until an operation completes.
8. **Avoiding unnecessary requests:** fetch on mount / on relevant state change (`useEffect` with explicit dependencies via `useCallback`-wrapped fetchers), not on every render; re-fetch only the data that actually changed (e.g. approving one change re-fetches that change set's detail, not the entire channel list).
9. **Never use a native browser dialog for anything this app itself designed (project owner's standing rule, 2026-09-21):** `window.confirm`/`window.alert`/`window.prompt` render unstyled, browser-chrome dialogs that break visual consistency with the rest of the app, and (discovered the same day) also block Chrome DevTools Protocol-driven browser automation used for this project's own live verification passes -- a native `confirm()` in `languages-manager.tsx` froze an in-progress live-verification session until the tab was closed. Use the shared `src/components/confirm-dialog.tsx` (`<ConfirmDialog title description? cancelLabel? confirmLabel confirmVariant? onCancel onConfirm />`) for any confirm-before-proceeding interaction, and an equivalent in-app banner/toast component (follow the existing `error`/`trackedLanguageNotice`-style inline banners already used throughout this codebase) for anything that would otherwise be an `alert()`. This applies to every confirmation, warning, or notice this app decides to show -- not only destructive actions -- so the whole instrument stays in one consistent visual style. `VideoDetailModal`'s own discard-unsaved-changes confirmation and `languages-manager.tsx`'s two remove-tracked-language confirmations both use `ConfirmDialog` -- follow that pattern rather than reaching for `window.confirm` out of convenience, even for a "quick" one-off dialog.
10. **Boolean ON/OFF settings use the shared toggle switch, not a native checkbox (project owner's standing rule, 2026-09-21):** use `src/components/toggle-switch.tsx`'s `<ToggleSwitch checked onChange disabled? label />` for any single boolean setting a user flips on or off (see `live-writes-settings.tsx`'s "Live writes" and "MCP connection" toggles) -- a horizontal track with an animated thumb (red circle, right = on; gray, left = off), not a styled `<input type="checkbox">`. This is specifically for ON/OFF *settings* -- a "select this row" checkbox in a list/table (e.g. `batch-manager.tsx`'s change-selection checkboxes, `languages-manager.tsx`'s tracked-language rows) is a different, genuine multi-select semantic and stays a real `<input type="checkbox">`. A setting whose risky direction is "on" (grants access, enables a real write, etc. -- the same category `ConfirmDialog` already exists for) should still gate that transition behind `ConfirmDialog`, exactly as `live-writes-settings.tsx` does for both of its toggles -- the switch component only changes the visual control, never the confirmation requirement above.

---

## 6.10 Adding import/export functionality

Reference: `src/lib/localization/adapters/xlsx.ts` (export) and `src/lib/changesets/import.ts` (import) — together the most complete example of this pattern in the repository. Since 2026-09-26 (`docs/roadmap/plans/SHARED_XLSX_MODULE_PLAN.md`), the genuinely generic spreadsheet mechanics both files need (cell-text reading, header-name indexing, generic sheet building/loading, byte/row-count safety limits, a generic key-value "Meta" sheet) live in `src/lib/shared-xlsx/`, reused by both — a new screen that also needs XLSX import/export should call into that module rather than re-implementing these mechanics, adding its own adapter file to `shared-xlsx/usage-inventory.test.ts`'s allowlist. Everything below that is domain-specific (which sheet/columns are required, what a row means, business validation) still belongs in the caller, exactly as this section already describes.

- **Workbook schemas:** define required sheets and columns explicitly; detect them by **header name**, not column position (`shared-xlsx`'s `buildHeaderIndex()`, called from `import.ts`, maps header text → column number) — this survives column reordering in a manually-edited workbook.
- **Backward compatibility:** a new export field should be additive (a new sheet or new column), and the importer must tolerate its absence from an older export rather than failing the whole import — see `import.ts`'s `readMetaSheet()`, built on `shared-xlsx`'s generic `readKeyValueSheet()`, which returns `null` for a workbook with no `Meta` sheet at all instead of throwing.
...
- **Parsing limits:** enforce a maximum file size and row count before/while parsing — the limit *values* (`MAX_WORKBOOK_BYTES`, `MAX_LOCALIZATION_ROWS`) are this domain's own tuning decision and stay in `import.ts`; the enforcement *mechanism* (`loadWorkbookFromBuffer()`, `assertDataRowCountWithinLimit()`) is generic and lives in `shared-xlsx` — an untrusted uploaded file must never be allowed to exhaust memory or hang the process (`docs/TECHNICAL_DEBT.md` RISK-01 tracks this check's known limitation).
- **Treat cell content as data, never as instructions:** read a formula cell's cached `result` only, never evaluate or re-interpret it — see `shared-xlsx`'s `cellText()`.
...
| A backlog item derived from `FUTURE_PHASES.md` is proposed, assigned, started, finished, or dropped | `docs/roadmap/BACKLOG.md` (via the `roadmap-backlog` skill, `.claude/skills/roadmap-backlog/SKILL.md`) — a `done` row must be accompanied by the matching `docs/ROADMAP_STATUS.md` update in the same pass, never a separately-forgotten follow-up |
