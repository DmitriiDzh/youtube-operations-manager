...
| BL-014 | Studio-parity S3: Languages tab — Studio-style table (Video / language count / last-modified) plus per-video language editor | `docs/roadmap/plans/STUDIO_PARITY_PLAN.md` §4 (Slice S3) | proposed | 2026-09-20 | Open design question (plan §5.1) before this can be assigned as scoped: restyle the existing "Localizations" tab in place, or add a second, separate tab covering the same underlying per-video-language data? Plan recommends restyling per `AGENTS.md` §D. Not yet assigned. |
