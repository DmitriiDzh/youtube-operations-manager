...
| **E5b** | 7.2's real deletion with multi-step confirm and 30-day-visible restore, wired to the "remove column" UI action when real data exists | No new write path needed after all — routes through the EXISTING `changesets.proposeLocalizationDeletion` (BL-036, backend already built 2026-09-21) extended to a bulk/multi-video form, per the owner's own instruction: "отправляет эти корректировки точно так же как мы отправляем информацию о дополнении информации" (send these corrections exactly the way we send addition info) | **Medium — reuses an existing, already-safety-modeled pipeline (approve → Gate-B-blocked Batch execution) rather than building a second one, which resolves BL-035's Open Question 2 by simply not needing to answer it. Real complexity is in the bulk-deletion service function (must not silently skip a video whose `defaultLanguage` matches, must compute the affected/skipped sets from both `existingLocalizations` AND `defaultLanguage`, not just the union `collectChannelLanguages` uses) and the warning UI.** | **Not yet built** — deliberately split from E5a per advisor's explicit "ship Slice A alone first" recommendation (2026-09-21) after the E1-E4b independent-review cycle's own lesson about shipping large, coupled changes in one pass. Owner has already described the intended UX (warn if real data exists, backup, route through the same pipeline as additions) — awaiting explicit assignment to implement. |
