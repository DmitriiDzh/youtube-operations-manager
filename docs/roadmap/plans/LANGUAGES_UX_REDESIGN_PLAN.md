...
| **E5a** | 7.2's tracked-language add/remove ONLY (display preference, no deletion) | Yes — additive `channels.target_languages_json` + 2 endpoints | **Low — no YouTube write, no new safety-critical surface. Explicitly separated from E5b by the advisor before implementation, specifically to avoid another multi-round oscillation like the E1-E4b independent-review cycle produced.** | **IMPLEMENTED 2026-09-21** (owner instruction, Telegram: "Давай пока добавим возможность добавить столбцы для новых языков... И возможность удалять колонки"). See `docs/ROADMAP_STATUS.md` for the full writeup. |
| **E5b** | 7.2's real deletion with multi-step confirm, wired to the "remove column" UI action when real data exists | No new write path needed after all — routes through the EXISTING `changesets.proposeLocalizationDeletion` (BL-036, backend already built 2026-09-21) extended to a bulk/multi-video form, per the owner's own instruction: "отправляет эти корректировки точно так же как мы отправляем информацию о дополнении информации" (send these corrections exactly the way we send addition info) | **Medium — reuses an existing, already-safety-modeled pipeline (approve → Gate-B-blocked Batch execution) rather than building a second one, which resolves BL-035's Open Question 2 by simply not needing to answer it.** | **IMPLEMENTED 2026-09-21 (BL-040).** `proposeLocalizationDeletion` unified into one function (`{channelId, language, videoIds?}` — omitted `videoIds` means "every video on the channel with a real localization"), computing `affectedVideoIds`/`skippedDefaultLanguageVideoIds` correctly from `defaultLanguage` independent of the `existingLocalizations` union (the undercount trap the advisor flagged). New route + UI: the "✕" on a language header with real data shows an explicit warning (what will happen, that a backup is captured automatically by the existing batch-preparation pipeline, that nothing is immediate) before proposing; the column is deliberately NOT untracked on confirm, since it stays visible via real data regardless until an actual write + re-sync. **Still not built, deliberately out of scope for this slice:** a "30-day-visible restore" UI — there is nothing to restore until Gate B is cleared and a real deletion can actually happen; `docs/TECHNICAL_DEBT.md` RISK-41 already tracks the backup-retention/purge side of this for when that day comes. |
...
2. **A second live write path — moot, per the owner's own later instruction (2026-09-21).**
   Rather than answering "yes/no, build a second write path," the owner asked for deletion to
   route through the SAME pipeline as adding a translation ("отправляет эти корректировки точно
   так же как мы отправляем информацию о дополнении информации"). E5b (§7.6) does exactly that,
   reusing the existing `changesets.proposeLocalizationDeletion` (BL-036) rather than building
   anything new -- this question no longer needs a yes/no answer, since nothing new is proposed.
   The rest of this note is kept for history/context. In short: `src/lib/batches/`'s write
