...
**Source of a release build:** each released version's canonical, self-contained source is a
`published/<version>/` folder inside this repository — a direct commit on `main` immediately
after that version's approved `dev → main` release merge, generated by
`scripts/publish-snapshot.mjs` (`docs/decisions/0003-published-release-snapshots.md`). It is an
allowlisted slice of the repository (no internal planning/process docs, no secrets, no build
artifacts) — safe to copy to another device or hand to another person. Populate the Syncthing
folder below *from* a `published/<version>/` folder (a plain copy); it is not built or maintained
independently of it.
