...
- **Languages** (Phase 3, extended in Phase 4; merged with AI Localization and restyled 2026-09-20, `docs/roadmap/plans/LANGUAGES_TAB_MERGE_PLAN.md`, formerly two separate tabs "Localizations" and "AI Localization") — `src/components/languages-manager.tsx` (renamed and rewritten from `localization-manager.tsx`, which is deleted, along with the now-absorbed `ai-localization-panel.tsx`): a "Generate with AI" section is now the primary, top-of-tab action (target languages, connection selector, per-video checklist, review/edit proposals, create Change Set — the exact same `ai-localization` API calls the old panel made, just relocated and reprioritized per the owner's decision that AI generation is the app's default localization path); XLSX import/export remains available as a secondary, collapsible bulk action lower on the page; a Studio-shaped video table (Video/Languages count/Last modified, replacing the old per-language ✓/— grid, which is still available per-video in the expandable detail row) with a "Generate with AI for this video" shortcut that pre-selects that video in the checklist above; and a change-set list filtered by three sub-tabs ("Все"/"В процессе"/"Одобрено") mapped onto `ChangeSet.status`, not onto any per-video state Studio's own UI assumes but this app's approval model doesn't have. "Одобрено" never implies a real YouTube write happened (Phase 5's write barrier is unaffected). `ChangeSetReview` gained an optional `onStatusChange` callback so the parent's separately-fetched change-set summary list stays in sync with an approve/reject action taken inside the open review panel.

Both follow the existing component conventions (Tailwind dark theme, same button/card styling as `ManualMode`). No existing tab, route, or component was modified beyond adding the new tab entries, their conditional render branches, and (for the 2026-09-20 merge) the `LocalizationOverviewRow.lastSyncedAt` field and `ChangeSetReview`'s optional callback described above.
