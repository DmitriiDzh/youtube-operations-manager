...
export async function POST(
  _request: Request,
  { params }: { params: Promise<{ channelId: string; changeSetId: string; changeId: string }> }
) {
  ...
  try {
    const { channelId, changeSetId, changeId } = await params;
    await channelAccess.assertActiveChannel({ userId: session.user.id, channelId });
    const result = await core.approveChange({ channelId, changeSetId, changeId });
